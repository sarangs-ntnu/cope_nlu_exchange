...
2. Inspect linear model weights from the TF-IDF baseline:
   ...
   # Fit a quick model to read weights
   ...
   PY
   ```
3. For transformer attributions, load your fine-tuned model (from the transformer tasks) and run Captum Integrated Gradients over a few teacher/course examples, then visualize token importances with matplotlib.
